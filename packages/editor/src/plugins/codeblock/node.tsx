...
import type { CodeBlockEditorProps } from '.';
import { defaultCodeBlockLanguage$ } from '.';
import { codeBlockEditorDescriptors$, nestedEditorUpdatedCommand } from '../core';
...
const CodeBlockEditorContextProvider: FC<{
  parentEditor: LexicalEditor;
  lexicalNode: CodeBlockNode;
  children: ReactNode;
...
export function useCodeBlockEditorContext() {
  const context = useContext(CodeBlockEditorContext);
  if (!context) {
...
const CodeBlockEditorContainer: FC<
  {
...
> = (props) => {
  ...
  if (!descriptor) {
    descriptor = codeBlockEditorDescriptors.find((descriptor) =>
      descriptor.match(defaultCodeBlockLanguage || '', props.meta || '')
    );
  }
