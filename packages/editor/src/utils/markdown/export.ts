import { isMdastHTMLNode } from '@/plugins';
import type { ElementNode as LexicalElementNode, LexicalNode, RootNode as LexicalRootNode } from 'lexical';
...
import { mergeStyleAttributes } from './merge-style';
      ...
      if (onlyChild.type === 'mdxJsxTextElement' && onlyChild.name === 'span') {
        onlyChild.attributes.forEach((attribute) => {
          if (attribute.type === 'mdxJsxAttribute') {
            const parentAttribute = (node as any).attributes.find(
              (attr: MdxJsxAttribute) => attr.type === 'mdxJsxAttribute' && attr.name === attribute.name
            );
            if (parentAttribute) {
              if (attribute.name === 'className') {
                ...
                parentAttribute.value = mergeStyleAttributes(
                  parentAttribute.value as string,
                  attribute.value as string
                );
              }
            } else {
              (node as any).attributes.push(attribute);
            }
          }
        });
  ...
  if (Object.hasOwn(node, 'children')) {
    ...
    nodeAsParent.children.forEach((child) => {
      if (child.type === 'mdxJsxTextElement' && child.name === 'u') {
        newChildren.push(
          ...[{ type: 'html', value: '<u>' } as const, ...child.children, { type: 'html', value: '</u>' } as const]
        );
      } else {
...
export function exportMarkdownFromLexical({
  ...
  return (
    toMarkdown(exportLexicalTreeToMdast({ root, visitors, jsxIsAvailable }), {
      extensions: [...toMarkdownExtensions, mathToMarkdown()],
      ...toMarkdownOptions,
    }) + '\n'
  );
}
