...
import type { NodeRef, Realm } from '@mdxeditor/gurx';
import { Cell, Signal, filter, map, scan, withLatestFrom } from '@mdxeditor/gurx';
import type {
  EditorThemeClasses,
  ElementNode,
  Klass,
...
/** @internal */
export const nestedEditorUpdatedCommand = createCommand<undefined>('NESTED_EDITOR_UPDATED_COMMAND');
...
/** @internal */
export const placeholder$ = Cell<ReactNode>('');

/** @internal */
export const autoFocus$ = Cell<boolean | { defaultSelection?: 'rootStart' | 'rootEnd'; preventScroll?: boolean }>(
  false
);
 ...
 * @group Core
 */
export const currentFormat$ = Cell(Format.Default);
 ...
 * @group Core
 */
export const editorRootElementRef$ = Cell<RefObject<HTMLDivElement> | null>(null);

/**
 * A reference to the content editable element.
 * @group Core
 */
export const contentEditableRef$ = Cell<RefObject<HTMLDivElement> | null>(null);
...
/** @internal */
export const composerChildren$ = Cell<ComponentType[]>([]);
...
/** @internal */
export const topAreaChildren$ = Cell<ComponentType[]>([]);
...
/** @internal */
export const editorWrappers$ = Cell<ComponentType<{ children: ReactNode }>[]>([]);
...
/** @internal */
export const nestedEditorChildren$ = Cell<ComponentType[]>([]);
      ...
      () => {
        theEditor.getEditorState().read(() => {
          ...
          if ($isRangeSelection(selection)) {
            ...
            setTimeout(() => {
              theEditor.dispatchCommand(nestedEditorUpdatedCommand, undefined);
            });
          }
        });
      },
...
export const viewMode$ = Cell<ViewMode>('rich-text', (r) => {
  ...
  r.sub(
    ...
    ([, editor]) => {
      editor?.dispatchCommand(nestedEditorUpdatedCommand, undefined);
    }
  );
});
